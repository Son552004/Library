<?php
	...
	if (isset($_POST['change_password'])) {
		...
		if (mysqli_num_rows($password_check_run) > 0) {
			...
			if (password_verify($current_password, $hashed_password_from_db)) {
				...
				if ($new_password === $confirm_password) {
					...
					if (mysqli_query($connection, $update_password_query)) {
						echo "<script type=\"text/javascript\">alert('Mật khẩu đã được đổi thành công!'); window.location.href = 'admin_dashboard.php';</script>";
						exit();
					} else {
						echo "<script type=\"text/javascript\">alert('Có lỗi xảy ra khi đổi mật khẩu: " . mysqli_error($connection) . "');</script>";
					}
				} else {
			...
			echo "<script type=\"text/javascript\">alert('Không tìm thấy tài khoản quản trị viên.');</script>";
		}
	}
?>
<!DOCTYPE html>
	...
	<div class="container">
		<div class="password-container">
			<form method="post" action="">
				<h3 class="text-center mb-4">Đổi mật khẩu Admin</h3>
